...
function tryEvalInitExpr(expr) {
  try {
    return eval?.(
      `(i, j, h, w) => { try { return (${expr}) } catch (e) { return 0 } }`,
    );
  } catch ({ name, message }) {
    console.log(
      `error ${name} evaluating init expr '${expr}' message '${message}'`,
    );
    return () => 0;
  }
}
...
function getInitFunc(init_params) {
  ...
  const scaled =
    useRange(init) && (min != 0 || max != 1)
      ? (i, j, h, w) => min + Math.max(0, max - min) * f(i, j, h, w)
      : f;
  const sparse =
    useDropout(init) && dropout > 0
...
function layernorm_(h, w, data) {
  ...
  const mean2 =
    data.map((x) => x ** 2).reduce((acc, x) => acc + x) / data.length;
  ...
  const loop = (args, infos, f) =>
    infos.length == 0
      ...
      : [...Array(infos[0].n).keys()].map((index) => {
          ...
          if (start < max) {
            // dead final block when size * n - max > size
...
export class Mat {
  ...
  getDispH() {
    ...
    return (
      this.H +
      this.params.layout.gap * (Math.min(n, Math.ceil(this.H / size)) - 1)
    );
  }
  ...
  getDispW() {
    ...
    return (
      this.W +
      this.params.layout.gap * (Math.min(n, Math.ceil(this.W / size)) - 1)
    );
  }
  ...
  getRangeInfo() {
    ...
    const absmax =
      use_absmin || viz.sensitivity == "local"
        ? local_absmax
  ...
  sizeFromData(x) {
    ...
    if (isNaN(size)) {
      ...
      if (this.n_size_from_data_errors <= 100) {
        console.log(
          `HEY x ${x} size ${size} absx ${absx} absmax ${absmax} absmin ${absmin} zsize ${zsize}`,
        );
        if (this.n_size_from_data_errors == 100) {
  ...
  colorFromData(x) {
    ...
    const light =
      min_light + (range * Math.sqrt(light_vol)) / Math.sqrt(absdiff);
  ...
  getGlobalAbsmax() {
    return this.params.getGlobalAbsmax
      ? this.params.getGlobalAbsmax()
      : this.absmax;
  }
  ...
  getData(i, j) {
    if (i >= this.H || j >= this.W) {
      console.log(
        `HEY i ${i} >= this.H ${this.H} || j ${j} >= this.W ${this.W}`,
      );
      return 0;
    }
  ...
  bumpColor(r = undefined, c = undefined) {
    ...
    this.setColorsAndSizes(r, c, undefined, (x) =>
      this.colorFromData(x).add(COLOR_TEMP),
    );
  }
  ...
  isRightSideUp() {
    ...
    const p = new THREE.Vector3(0, -1, 0).applyQuaternion(
      this.group.getWorldQuaternion(q),
    );
    const c = new THREE.Vector3(0, 1, 0).applyQuaternion(
  ...
  setLegends(size = undefined, shape = undefined) {
    ...
    if (
      ...
      this.legend_state.H == H &&
      this.legend_state.W == W
    ) {
      return;
    }
    ...
    if (size > 0) {
      ...
      if (shape && this.params.deco.shape_info) {
        ...
        } = this.getBlockInfo();
        {
          ...
          this.hdim_text.geometry.rotateZ(
            ((place == facing ? 1 : -1) * Math.PI) / 2,
          );
  ...
  if (p.count === undefined) {
    p.count =
      p.matmul === false
        ? 0
        : 1 +
          ensureChildCounts(p.left).count +
          ensureChildCounts(p.right).count;
...
export class MatMul {
  constructor(params, context, init_viz = true) {
    ...
    if (this.D != height(params.right)) {
      console.log(
        `HEY left width ${this.D} != right height ${height(params.right)}`,
      );
    }
  ...
  getDispD() {
    ...
    return (
      this.D +
      this.params.layout.gap * (Math.min(n, Math.ceil(this.D / size)) - 1)
    );
  }
  ...
  initLeftViz() {
    ...
    if (this.params.layout.polarity.startsWith("positive")) {
      ...
      this.left.group.position.x = this.params.layout[
        "left placement"
      ].startsWith("left")
        ? -this.getLeftScatter()
  ...
  initRightViz() {
    ...
    if (this.params.layout.polarity.startsWith("positive")) {
      ...
      this.right.group.position.y = this.params.layout[
        "right placement"
      ].startsWith("top")
        ? -this.getRightScatter()
        : this.getExtent().y +
          this.right.getExtent().z +
          this.getRightScatter();
    } else {
      // negative
      this.right.group.rotation.x = -Math.PI / 2;
  ...
  initResultViz() {
    this.result.initViz();
    this.result.group.position.z = this.params.layout[
      "result placement"
    ].startsWith("back")
      ? this.getExtent().z
      : 0;
  ...
  updateLabels(params = undefined) {
    if (params) {
      ...
      this.params.deco["interior spotlight"] =
        params.deco["interior spotlight"];
    }
    ...
    const interior_spotlight = this.params.deco["interior spotlight"]
      ? spotlight
      : 0;
  ...
  getBoundingBox() {
    const get_bb = (mm) => {
      ...
      const max = mm.group.localToWorld(
        new THREE.Vector3().copy(mm.getExtent()),
      );
      const swap = (d) => {
        const temp = min[d];
        min[d] = max[d];
  ...
  getAbsmax() {
    return Math.max(
      this.left.getAbsmax(),
      this.right.getAbsmax(),
      this.result.getAbsmax(),
    );
  }
  ...
  initAnimation(cb = undefined) {
    ...
    this.alg_join = () => {
      const lalg =
        !this.params.left.matmul || left_done
          ? "none"
      ...
      const ralg =
        !this.params.right.matmul || right_done
          ? "none"
      ...
      return alg == "vmprod" && or_none(lalg, "vmprod") && ralg == "none"
        ? "vmprod"
        : alg == "mvprod" && lalg == "none" && or_none(ralg, "mvprod")
          ? "mvprod"
          : alg == "vvprod" &&
              or_none(lalg, "mvprod") &&
              or_none(ralg, "vmprod")
  ...
  getAnimIntermediateParams(name) {
    ...
    delete params.name;
    params.viz.sensitivity == "superlocal" &&
  ...
  getVmprodBump(sweep) {
    ...
    this.grid(
      ...
      (
        ...
        const vmp = new Mat(
          data,
          this.getAnimIntermediateParams(
            this.params.name + `.vmp[${ii}, ${ki}, ${ji}]`,
          ),
          this.context,
          true,
        );
        vmp.hide();
        const z =
          polarity < 0 ? this.getExtent().z - k - gap * ki : k + gap * ki;
    ...
    return () => {
      ...
      this.grid(
        ...
        (
          ...
          if (curi < ix && curj < jx) {
            util.updateProps(vmp.group.position, {
              x: j + ji * gap + curj,
              y: gap + i + ii * gap + curi,
            });
            vmp.reinit((ki, ji) =>
              this.ikjmul(i + curi, k + ki, j + curj + ji),
            );
          }
        },
      );
      ...
      this.grid(
        "ij",
        ({ start: i, extent: ix }, { start: j, end: je, extent: jx }) => {
          curi < ix &&
            curj < jx &&
            results.forEach((r) =>
              r.show(i + curi, sweep ? j + curj : [j, je]),
            );
        },
      );
    ...
    return () => {
      ...
      this.grid(
        ...
        (
          ...
          if (curi < ix && curj < jx) {
            util.updateProps(mvp.group.position, {
              x: gap + j + ji * gap + curj,
              y: i + ii * gap + curi,
            });
            mvp.reinit((ii, ki) =>
              this.ikjmul(i + curi + ii, k + ki, j + curj),
            );
          }
        },
      );
      ...
      this.grid(
        "ij",
        ({ start: i, end: ie, extent: ix }, { start: j, extent: jx }) => {
          curi < ix &&
            curj < jx &&
            results.forEach((r) =>
              r.show(sweep ? i + curi : [i, ie], j + curj),
            );
        },
      );
  ...
  getVvprodBump(sweep) {
    ...
    this.grid(
      ...
      (
        ...
        util.updateProps(vvp.group.position, {
          x: j + ji * gap,
          y: i + ii * gap,
          z,
        });
    ...
    return () => {
      ...
      if (oldk >= 0 && !this.params.anim["hide inputs"]) {
        sweep
          ? this.grid(
              "kj",
              ({ start: k, extent: kx }, { start: j, extent: jx }) => {
                oldk < kx &&
                  oldj < jx &&
                  this.right.setColorsAndSizes(k + oldk, j + oldj);
              },
            )
        ...
        sweep
          ? this.grid(
              ...
              ({ start: k, extent: kx }, { start: j, extent: jx }) => {
                curk < kx &&
                  curj < jx &&
                  this.right.bumpColor(k + curk, j + curj);
              },
            )
          : this.grid("k", ({ start: k, extent: kx }) => {
        ...
        (
          ...
          if (curk < kx && curj < jx) {
            const z =
              polarity > 0
                ? gap + k + ki * gap + curk
                : extz - gap - k - ki * gap - curk;
            util.updateProps(vvp.group.position, { x: j + ji * gap + curj, z });
            vvp.reinit((iii, jii) =>
              this.ikjmul(i + iii, k + curk, j + curj + jii),
            );
          }
        },
      );
      ...
      this.grid(
        ...
        (
          { start: k, extent: kx, index: ki },
          { start: j, end: je, extent: jx },
        ) => {
          if (curk < kx && curj < jx) {
            const running_dp = (ii, ji) => {
              const x =
                this.left.getData(ii, k + curk) *
                this.right.getData(k + curk, ji);
            ...
            const pw_epilog_dp = (ii, ji) =>
              this.applyPointwiseEpilog(pre_epilog.get(ii, ji));
            results[0].reinit(
              pw_epilog_dp,
              undefined,
              undefined,
      ...
      if (sweep) {
        this.grid("kj", ({ extent: kx }, { start: j, end: je, extent: jx }) => {
          if (curk < kx && curj < jx) {
            results[0].reinit(() => 0, undefined, undefined, [
              j + curj + 1,
              je,
            ]);
          }
...
export function fixShape(h, w, p, anc, root) {
  ...
  anc.length > 1 &&
    fixShape(height(pp.left), width(pp.right), pp, anc.slice(1), root);
}
...
export function genExpr(p) {
  ...
  return p.name == expanded || p.name == named
    ? expanded
    : `${p.name} = ${expanded}`;
}
